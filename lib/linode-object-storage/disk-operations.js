...
module.exports.exists = function exists(filePath, callback) {
  log.message(log.INFO, "JJG: Got exists()");
  
  //hoRes = los.headObject({ Bucket: bucket, Key: filePath }, callback); 

  //log.message(log.INFO, "JJG: headObject result: " + hoRes);
  //console.log(hoRes);
...
module.exports.write = function writeFile(...args) {
  ...
  // JJG DEBUG
  log.message(log.INFO, "JJG: Got write");
  log.message(log.INFO, "JJG: filePath: " + filePath);
  
  if (typeof contentType === 'function' && !callback) {

    log.message(log.INFO, "JJG: weird contentType function thing")
  ...
  if (!contentType && typeof contents === 'string') {
    ...
    log.message(log.INFO, "JJG: other contentType thing");
